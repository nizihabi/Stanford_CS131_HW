...
def rgb_decomposition(image, channel):
    ...
    image[..., color_code[channel.lower()]] = 0
...
def lab_decomposition(image, channel):
    ...
    ### YOUR CODE HERE
    color_code = {'l': 0, 'a': 1, 'b': 2}
    lab = (lab + np.abs(np.min(lab)))
    lab = lab / np.max(lab)
    lab[..., color_code[channel.lower()]] = 0
...
def hsv_decomposition(image, channel='H'):
    ...
    ### YOUR CODE HERE
    color_code = {'h': 0, 's': 1, 'v': 2}
    hsv = np.array(hsv)
    hsv[..., color_code[channel.lower()]] = 0
...
def mix_images(image1, image2, channel1, channel2):
    ...
    out[:,0:out.shape[1] // 2] = image1[:,0:image1.shape[1] // 2]
    out[:,out.shape[1] // 2 :] = image2[:,image2.shape[1] // 2 :]
    ### END YOUR CODE
